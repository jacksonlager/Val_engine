...
Status: in progress. This file is updated as work proceeds so that it survives an interruption.
...
### D-8 — A same-terms extension's date regressed to the old clock one quarter later (fixed, this commit)
...
### D-9 — 48.8 of booked NAV vanished at the quarter boundary (fixed, this commit)
...
### D-10 — A duplicated activity row was applied twice (fixed, this commit)
...
### D-11 — An item opened earlier in the quarter was not resolved by a later event in the same quarter (fixed, this commit)
...
| `build/<quarter>/` | What `hc-valuation build` emitted for each quarter: report, workbook, CSVs, `run.json`, and the next quarter's input (`portfolio_<next>.xlsx` + `open_items_carry.yaml`). |
...
the run (`8a6eef3`, `6a6bd97`, `5bb5f24`). First-run results:
...
- **Suite:** 1,116 passed at the last full run before the final wiring fix; the final run is recorded in the
  "Final state" section below. 69 new tests across nine files (listed per defect above, plus
