...
## Follow-up (after the overnight run): keep the test data out of the real book, and a hard-coding review

- **Synthetic workbooks are never offered from the real book.** `discover()` lists `data/*.xlsx` and
  `data/quarters/**` but skips anything marked synthetic (the `SYNTHETIC TEST DATA` sheet or SYNTHETIC in the
  file name) unless the server was started on a synthetic workbook or `HC_INCLUDE_SYNTHETIC=1`. Synthetic is a
  marker, not a location: a real `portfolio_Q4_2026.xlsx` under `data/quarters/2026Q4/` is a real book and records
  into the committee ledger (`data/`). The roll-forward now carries the marker sheet through `build`, and the
  chain script marks the Q4 input it emits from the real Q3 book, so no emitted file from the chain can pass
  as a real quarter. On the real book the select now shows only Q3 2026 and hides itself.
- **The default policy follows the workbook.** `RunPaths.default(policy=None)` picks `rules/<YYYY>Q<n>.yaml` for
  the workbook's own quarter when it exists (else the base policy, and X-922 says so). Verified: the Q4 book
  `build` emits validates with no `--policy` and zero blocking issues.
- **Hard-coding review of `src/` and both front ends.** Company names appear only in two comments; every card
  text ("why flagged", points, suggested next step) is generated by the rules from the workbook, and the
  recommendation cache is keyed by a hash of the case facts, so new data yields new answers — the three synthetic
  quarters are the proof. Fixed: the closing-price dialog's literal `2026-09-30` fallback (the IPO finding now
  carries `measurement_date`, as X-113 does; golden regenerated for that one evidence key), the executive
  footer's "September 2026 assessment" sentence (now the snapshot's quarter and date), and the rule catalogue's
