...
**6. Claude only adds.** The same rule binds both uses (note reader, recommender): a model may explain, classify or pick among the engine's priced options, never produce a number, lower a severity or unblock a position. The note reader: Claude reads each activity row's free text against the case
