...
function SliceTip({ active, payload }: { active?: boolean; payload?: { payload: CompositionSlice }[] }) {
  ...
  return (
    <div className="tip">
      ...
      <div className="text-muted">{pct(s.share)} of booked NAV · {plural(s.count, "position")}</div>
    </div>
  );
}

function SliceChart({ title, rows, ordered }: { title: string; rows: CompositionSlice[]; ordered?: boolean }) {
  const max = Math.max(1, ...rows.map((r) => r.nav));
  return (
    <div className="frame px-4 pt-3.5 pb-3">
      <div className="flex items-baseline justify-between">
        <div className="text-[13px] font-medium text-ink">{title}</div>
        <div className="text-[11px] text-muted">{ordered ? "booked NAV, largest first" : "booked NAV, largest first"} · label = share</div>
      </div>
      <div className="mt-1">
        <ResponsiveContainer width="100%" height={rows.length * 26 + 16}>
          <BarChart data={rows} layout="vertical" margin={{ top: 6, right: 96, bottom: 0, left: 0 }} barCategoryGap={6}>
            ...
            <Tooltip content={<SliceTip />} cursor={{ fill: "var(--accent-wash)" }} />
...
export function Composition({ view }: { view: ExecView }) {
  ...
  return (
    <Section
      ...
      title="Where the booked NAV sits"
      aside={
        <>
    ...
    >
      <div className="grid grid-cols-12 gap-3">
        <div className="col-span-7 max-[1180px]:col-span-12"><SliceChart title="By sector" rows={c.by_sector} /></div>
        <div className="col-span-5 max-[1180px]:col-span-12 flex flex-col gap-3">
          <SliceChart title="By stage" rows={c.by_stage} />
          <div className="frame px-4 pt-3.5 pb-4 flex flex-col gap-3">
            ...
            <div className="border-t border-hair pt-3 flex items-baseline justify-between gap-4">
              ...
              <div className="text-[12px] text-ink2">
                ...
                of booked NAV in the ten largest positions
              </div>
            </div>
