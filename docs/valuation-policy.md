...
| **M-040** | IPO / direct listing | `mark = ownership_after × market_cap(measurement_date) × (1 − lockup_discount)` | **9/30 close, not the IPO print.** L3 → L1. Default lockup discount 0% (ASU 2022-03: a contractual sale restriction is not a characteristic of the security; blockage on a Level 1 price is separately prohibited). Always BLOCK. `Direct Listing` dispatches here too. A lock-up item (`ipo_lockup_days`) opens on an IPO unless the row says the shares are not restricted ("not subject to a lock-up", "freely tradable"); a direct listing has none unless the row says it does. → Drayvenn. |
...
| **M-071** | Term sheet withdrawn | mark unchanged; the open term sheet and its indicated alternative are closed | A signed proposal fell through before closing. Nothing was booked from it, so nothing is unwound — but it is evidence about the last-round price and the raise is still ahead: always REVIEW (X-125). Figures in the note (cash, burn) raise X-126. |
| **M-072** | Operating update | mark unchanged | No transaction. The figures belong on the Portfolio tab, where the screens read them; always REVIEW (X-126) so a person updates the tab or confirms it is current. |
...
| **M-051** | Deal terminated (a break fee on the row goes to realized; the stake is unchanged) | `mark = ownership × latest_post` (back to the last-round basis); drops `pending_acquisition`; anchor unchanged | X-114 REVIEW. An announcement and its termination in one quarter: the announcement is superseded. |
...
| **M-080** | Stale-round comps calibration | `equity_mark × median over the same public names of (name's multiple now ÷ at the round month)`, bounded ±35% | **On, gated to an observed comps history** (`require_live_history`): the vendor-shaped fixture never calibrates; `--provider live` does. Writes the `calibrated_to_comps` alternative only, never the proposal. Section 2b. |
...
**What it computes.** After the roll, for every active Level 3 position with an ARR whose price anchor is at least `calibration.min_age_months` (24) old, that is not priced by a signed deal or the buyer's shares, and whose sector has an observed comps history covering the measurement month and the round month:

```
for each public name priced in both months:
    then_i   = median of the name's EV ÷ TTM revenue over round_month ± anchor_window_months   # 1
    now_i    = median of the same over measurement_month ± anchor_window_months
    ratio_i  = now_i ÷ then_i
factor      = median(ratio_i)  over the names priced at both ends (≥ min_same_set_names, 3)
factor      = clamp(factor, 1 − bound_pct, 1 + bound_pct)          # bound_pct: 0.35
calibrated  = equity_mark × factor
```

The re-rating is measured **name by name over the same names** (`engine/rerating.py`), never as one basket median divided by another. A basket "median" of five names is the third-ranked name, and on the shipped history the name at the median differed between the round month and today in 29 of 42 calibrations — every AI/ML calibration was one micro-cap's own trough-to-peak, every Enterprise SaaS one was Adobe-then over Workday-now. It also moves whenever a name enters the basket: Ostrella's Space & Defense round month (2021-08) had three listed names and today has five, and the basket-median ratio read +15% where the three names present both times read −20%. The same-set median of per-name moves is immune to both. Each end is a name's median over ±1 month so a single month-end print in a thin stock does not set a private mark's alternative. When the round month has no values at all, the nearest month inside `round_month_tolerance` (±3) is used and recorded.

The M-080 step records `method`, `n_names`, every name's `then`, `now` and `ratio`, `factor_raw`, `factor_bounded`, `bound_hit`, `comp_month_used`, `priced_as_of` (the day the measurement month was priced when it is not a month-end) and whether the alternative sits below invested cost, so a workpaper reader sees the five moves, the median, and whether the bound rather than the comps set the number. The step's formula line writes the arithmetic out: `AI 3.60÷8.04=0.45 · BBAI 12.39÷3.71=3.34 · NVDA 17.82÷30.63=0.58 · PLTR 68.87÷15.30=4.50 · SOUN 14.22÷10.27=1.38; median of 5 = 1.385 → cap … $24.10M × 1.3500 = $32.53M`. On the committed history M-080 produces 42 indications, 23 of them set by the ±35% limit.

It writes `calibrated_to_comps` into `alternative_marks` and appends an M-080 step whose prior and new value are both the proposal. **It never writes the proposal.** The base mark stays at the last round; the calibrated figure is a labelled alternative a reviewer can choose, and choosing it is an E-01 override like any other. The option's label carries its own arithmetic and the cap — "Calibrate the mark to public comps: $20.60M × 0.6605 = $13.61M (−34.0% since the round)" or "… $1.30M × 0.6500 = $0.85M — comps moved ×0.637 (−36%), capped by policy at −35%" — so the cap is on the decision surface and on the ledger, not only in the audit chain. It has nothing to do with the ±20% multiple sensitivity in the Summary — that is `rollup.py`, a portfolio-level shock on multiple-exposed NAV, and it runs with or without M-080. The three share one exposure test, `multiple_exposed`: a Level 3 mark with ARR at or above `multiple.min_arr` whose price rests on a round; Level 1, pre-revenue, deal-priced and terminal positions are never moved by a multiple regime.

**What is compared to what.** The public side is enterprise value over trailing-twelve-month revenue, point-in-time. For M-080 only the ratio of ratios matters, so the definitions cancel — on the assumption that the company's own revenue multiple would have moved with its sector's, which is the whole content of the indication. X-401 / X-402 are a different comparison: the round's post-money over the company's ARR against 0.5×–2× that public median. Post-money is an equity value that includes the cash just raised (not EV), ARR runs ahead of trailing revenue for a company growing fast, and no growth adjustment is made; the flag text says so, and that is why both are MONITOR screens and never a valuation.
...
**On the fixture, the bound sets the number — so the fixture never calibrates.** The PitchBook-shaped fixture trends hard: with the gate waived, **31 of the 42 candidates pin at +35% and two more at −35%**; only nine land between. On the stub it is the bound, not the comps, that sets most alternatives — a property of the fixture, not the rule. Policy 2026Q3-0.2 therefore ships calibration **on** but gated: `require_live_history: true` means M-080 runs only when the sector's comps carry an observed source (`live:edgar+yahoo`), and the fixture leaves every alternative absent. Under `--provider live` the baskets carry `months_of_history: 96` (Yahoo `range=10y`), so the history can reach the round months of this book's stale positions (48–71 months old) — on the condition that at least `min_constituents` (3) of the sector's basket were listed and had filed in that month; a month below that has no value, the ±3-month tolerance covers most such gaps, and the M-080 step records how many names stood behind each of the two multiples (`n_constituents_at_round`, `n_constituents_now`) so a reviewer sees when a 2020 value rests on three names and a 2026 one on five. Every month of the history is point-in-time: the revenue, share count and balance sheet used are those *filed* by that month's end, so the multiple in a round month is the one the market saw when the round was priced, not one restated later. Where it does not, the M-080 step and the alternative are simply absent and the "calibrate" suggestion does not appear.

**The sensitivity view.** The ±20% shock the brief asks for is `rollup.sensitivity`, reported on every multiple-exposed mark and on the software sectors alone (`sensitivity.software_sectors` names what "software multiples" means); the review tool's Movement page carries a *Sensitivity view* with a slider from −20% to +20% that interpolates the same arithmetic — NAV(s) = booked + exposed × s — by sector, by fund and by position, with a scope switch between all sectors and the software sectors. What moves is the equity leg of every round-priced Level 3 mark with ARR at or above the floor, times `multiple_exposed_share`: 1 for a round price, the break-branch weight for a probability-weighted deal mark (Gryphonel: 0.10 × $3.50M ÷ $4.66M = 7.5%), 0 for a full deal price or the buyer's shares; a note leg at cost is excluded and a mark booked below its note leg contributes nothing, never a negative. The screen states in dollars what is held flat — the listed position, the deal-priced marks, the notes, and (in software scope) the non-software sectors — and says that a round-priced mark re-rating one-for-one with its sector is the exposure, not a forecast: a last-round price does not actually move until the next round.

**Observed sensitivity (`comps_move`).** The same history answers a second question the brief asks — what the book looks like if software multiples move — with what they actually did: each sector's same-set re-rating (the median of each name's own now ÷ then, the statistic M-080 uses, window 0) from three months before the measurement month to the measurement month, applied to that sector's multiple-exposed NAV. Each row shows the basket medians for context beside the same-set move, because the two can disagree by sign: over Q3 2026 the Infrastructure basket median fell 17.46× → 13.23× (−24%) while the five names' own moves had a median of +4.8% — the basket's third-ranked name changed identity. It is alternative arithmetic like M-080, rendered on the Sensitivity view under the slider and in `run.comps_move`, labelled fixture or live by the source of each sector's history, and carrying `priced_as_of` when the measurement month is not yet a month-end.
...
**How a reviewer uses it.** X-202 (round older than 48 months), X-106 (flat extension without price discovery) and X-405 (stale price that a live screen argues with) each carry a "Calibrate the mark to public comps" suggestion that books `calibrated_to_comps`. On X-405 it is offered only when the calibration points the way the screen does — a mark flagged as carried *above* the multiple screen is not offered a calibration that lifts it; the finding says which way the calibration would move and that it is withheld. Accepting it is an E-01 override addressed to that rule, under a named approver, with `source_suggestion` on the ledger record. If the alternative was not produced (calibration off, sector history missing, round too old for the live history) the suggestion is dropped rather than shown with no number.
...
| **X-111** | Cash distribution received | MONITOR / REVIEW | M-022: stake unchanged, realized up. Review when the distribution exceeds both the carrying value and invested capital on an Active position: that is a sale, a liquidation or a return of capital typed as a distribution, and "treat it as the exit it looks like" is a priced option. |
...
| **X-119** | Cheque does not reconcile | REVIEW | M-010. When the text states the round size ("$26.0M Series B"), the stake after the round should be the prior stake diluted by round ÷ post plus cheque ÷ post; a stake more than `indications.cheque_ownership_tolerance` (1.5pp) and `cheque_price_tolerance` (10%) away from that is a finding (a wrong cell, a pre-money read as a post, different terms). Without a round size, `cheque ÷ Δownership` is the price HC paid — meaningful only when the stake bought is at least `cheque_check_min_ownership_delta` (1pp), and allowing ± one ownership rounding unit each side. Never a mark; "book at the implied stake/price" is a priced option. |
...
| **X-124** | Note converted in the round | REVIEW | M-010 / M-040 when a note leg carried at cost converts: the leg is removed, the whole stake is marked at the round price, and the note's principal is in invested capital exactly once. Replaces X-107 / X-108 on the position — the bridge questions are settled; what remains is the conversion itself (cap or discount, accrued interest, share count against the closing cap table). Family `treatment`. |
| **X-125** | Term sheet withdrawn | REVIEW | M-071. Hold the last-round mark, or mark down to cost pending a new raise: the engine cannot tell a withdrawal on terms from a withdrawal on the company. Family `treatment`. |
| **X-126** | Operating metrics reported in prose | REVIEW | M-071 / M-072 when the note mentions cash, burn, runway, revenue, ARR, headcount or margin. The engine never reads a number out of a sentence, so the runway and growth screens still run on the Portfolio tab's columns, which the note may supersede. Action: update the tab from the note (or confirm it is current) and rerun. Family `liquidity`. |
| **X-120** | New position entered at cost | MONITOR | M-014. |
| **X-134** | A figure on the row the rule does not use | REVIEW | The row carries a number in a column its event's rule never reads (proceeds on a term sheet, a cheque on a distribution, a stake on a dividend, a recovery estimate on a shutdown). The figure went nowhere; the reviewer refiles the row or clears the cell. Family `data`. |
| **X-135** | The row's label contradicts its price | REVIEW | M-010: the row says "down round" while the post-money is at or above the last round. Family `data`. |
| **X-925** | Portfolio row the engine could not read | BLOCK | Ingest: a Status word that is not Active / Acquired / Shut Down (synonyms such as Exited, Public, Dissolved are read and recorded) or a blank Latest Round with no First Investment date. The row is held as Active and blocked; the rest of the book values. |
| **X-927** | Row repeats the book's latest round | BLOCK | Ingest: a priced round dated on the position's Latest Round at the same post-money (and stake) is the record the Portfolio tab already carries; applying it would count HC's cheque twice. Refused. |
| **X-924** | New investment on a company already held | BLOCK | Ingest: a `New Investment` row that repeats the position the Portfolio tab already carries (same round price, same stake — or a "first cheque" that leaves the stake no higher) is refused; applying it would add HC's money again. The position blocks (X-900) until one of the two records is removed. A New Investment at a different price and a higher stake is a follow-on filed under the wrong label and applies (M-014). |
...
| **X-926** | Dates out of order on a position | REVIEW | Ingest: `First Investment` is after `Latest Round`. One of the two dates is wrong; the mark does not rest on either, but the staleness clock runs from the second, so a person confirms which before that clock is trusted. |
...
exceptions:
  ...
  multiple:   { mode: relative_to_comps, absolute_high: 30, absolute_low: 3,    # absolute | relative_to_comps (live sectors)
...
note_screen:
  terms: [escrow, holdback, earn-out, milestone, contingent, participating, ratchet, preference,
          litigation, restated, bankruptcy, conversion, warrant, pay-to-play, cram-down, lock-up,
          related party, going concern, covenant, default, …]   # 75 terms; the full list is in rules/2026Q3.yaml
open_items:
...
declarative:                          # the whitelist a custom_rules formula is parsed against
...
| **E-03** | Run manifest + determinism | run_id = hash(sha256(input), policy content, engine_version, decisions ledger + carried items, market source), sha256(input), policy_version, engine_version, measurement_date, generated_at. Same input and same decisions → byte-identical output; a new override → a new run_id. |
...
**The honest risk in an unrecognised event** is that it gets absorbed by a rule it merely resembles. The engine refuses to: M-999 halts the position at its prior mark and books nothing, and the rule for the new type is written into the policy file by a person with an approver and an effective date. That is deliberately more friction than a path doesn't silently create policy. After the same treatment is accepted three times, the queue suggests promoting it — by then it's a pattern, not a guess.
