...
2. **Relative screening on the live comps.** M-080 already calibrates against the live history; the next step is `multiple.mode: relative_to_comps`, so X-401/402 screen each mark's implied multiple against its sector's live median (2.0× / 0.5×) instead of the absolute 30× / 3× — a policy change to bump the version for, once the baskets have been reviewed name by name with the deal team.
