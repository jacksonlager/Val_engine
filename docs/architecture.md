...
flowchart TB
    ...
    subgraph NOTES["Note reader  (impure, before the engine)"]
        NR["notes/reader.py + notes/catalogue.py<br/>each activity row's free text against the 29-case catalogue<br/>quotes verified · any figure it names withheld · never a number"]
        NRC[("data/note_reads/<br/>one reading per row, cached")]
        NR --> NRC
    end
    ...
    subgraph ENG["engine/  — PURE: no file, network or clock access"]
        ...
        MARK["marking.py<br/>M-000 … M-072, M-999, M-080<br/>each step appends a MarkStep"]
    ...
    subgraph REV["Review surface"]
        API["api/app.py  FastAPI<br/>GET /api/run · /api/companies · /api/rules · /api/rationale · /api/market · /api/history · /api/signals · /api/sources · /api/published · /api/exec · /api/workbooks · /api/publish/readiness · /api/output-workbook<br/>POST /api/overrides · /api/rerun · /api/publish · /api/upload · /api/workbook · /api/reset · /api/market/refresh"]
    ...
    RD -- "activity rows" --> NR
    NR -- "readings → X-126/130/131/132" --> ENG
    LED --> OVR
    CARRY --> OI
    POL --> ENG
    ENG --> RUN
...
| (c) A decision or rerun through the API | `POST /api/rerun`; every ledger write (`POST /api/overrides`, `POST /api/publish`) | Re-executes the pipeline against the current ledger and policy — this is how an override recorded through the UI becomes a new `booked_nav`. |
...
| EDGAR + Yahoo (live, free; Stooq as the alternative price source) | Sector EV/TTM-revenue multiples now and by month (`comps`, `comp_history`) computed from SEC XBRL fundamentals (revenue, shares, net cash) and daily closes from a pluggable price source — Yahoo Finance's chart API by default, Stooq selectable with `--price-source` / `HC_PRICE_SOURCE` but currently behind a browser-verification wall — for the public baskets in `rules/comps_baskets.yaml` | The same slots as PitchBook: X-401/402 in `relative_to_comps` mode, M-080 calibration | `connectors/edgar.py`, `prices.py`, `stooq.py`, `fetch.py`, `cache.py`, `live.py` (`PublicCompsProvider`); cached under `data/market_cache/<as_of>/` (`prices/` and `splits/` keyed by `meta.price_source`); per-constituent fall-back to the fixture below `min_constituents`; a browser-verification page from a source is reported, never bypassed. Three inputs are refused and counted rather than used: a share count the filer stopped reporting more than 450 days before the date valued, a month whose split-adjusted close and as-filed share count cannot be put on one basis because no split history is on file, and a negative enterprise value. The report behind `GET /api/market` and `hc-valuation market` — see `docs/market-feed.md` |

Provider selection is an explicit argument (`--provider`), else the `HC_MARKET_PROVIDER` environment variable, else `stub`. `live` (alias `stooq`) reads `data/market_cache/<measurement date>/` and makes no network call when the cache is complete (the manifest label is `live:edgar+<price source>`, e.g. `live:edgar+yahoo`); otherwise it fetches what is missing (`--refresh-market` refetches everything, closes and split histories together — a cache written before split capture keeps its measurement-date values but has no monthly history until then). Every failure is caught per constituent and listed in the report; a sector with fewer than `min_constituents` priced names keeps the fixture value, and if no sector reaches live the manifest label says `stub` rather than pretending — a live label never sits over fixture numbers. `assemble_market_data` returns the `MarketData`, the manifest label and the market report together (`MarketAssembly`); swapping stub → live changes no line in `engine/`.
...
`engine/` imports nothing from `api/`, `ingest/`, `connectors/`, `export/` or `pipeline`. It does no file, network or clock access; the measurement date comes from `config.quarter`, and `generated_at` is an argument. Three things follow:
...
3. **The model layers cannot leak into a number.** The note reader runs before the engine and hands in readings; the recommender runs after it and chooses among options the engine already priced. Neither is called from inside `run_valuation`, and the only way either changes a mark is by a person recording an override (E-01), which is an input to the *next* run.
...
**Layer 3 — the policy file (`rules/2026Q3.yaml` → `config.py` `RuleConfig`).** Every threshold, the quarter window, the reporting lag, the escalation rule, the note-screen terms, the open-item ageing limits, the declarative whitelist, and `custom_rules` (rules written into the policy file, E-09). Strict: an unknown key raises. `inherits` lets `2026Q4.yaml` override only what changed, so the diff between quarters is the policy audit trail. There is no threshold literal in engine code.
...
Every BLOCK and REVIEW flag also carries one to three `suggestions` — the resolutions the rule can put a number on (ratify as proposed, hold the prior mark, the full deal value, the secondary price, cost as a floor, an alternative mark), each a one-sentence label, two reasons and the booked mark it produces. Rules write them with a *basis* (`Suggest` in `state.py`) and `Working.resolved_flags` fills the numbers in only once the roll is finished and the proposal is final; two suggestions that book the same figure collapse to one, and one naming an alternative mark that never materialised is dropped. Accepting a suggestion in the review tool is an ordinary E-01 override addressed to that rule id (`source_suggestion` on the ledger record), confirmed under a named approver, so it re-runs into the booked mark, the disposition, the totals, the exports and the archive like any committee decision — and the proposal is never rewritten. **New Activity first.** The first tab lists every position the quarter's activity tab touched — a row per event with its date, sheet and row number, the mark it produced (prior → new) and the engine's sentence for the rule that applied it — ordered blocked first, then in the order the rows were entered, Ready positions included. A reviewer who has read this page has been informed of every decision the new information produced before reaching the positions the engine left alone; the same positions carry a *New activity* label on the Queue. The cards are the same component as the Queue's, so a decision can be taken on either page.

**The exception card.** One card per position on the queue, in the order a reviewer works. The status on it is `readiness` — *Blocked* (an input is missing; no supported mark), *Needs Review* (a proposal exists; judgment needed), *Ready* — one bucket per position, computed in `engine/readiness.py`; a watch item is a tag beside it, not a bucket, and `approval` (*Not approved* → *Decision recorded* → *Approved and published*) is a third thing again, so the summary strip reads *Prior → Proposed (or Provisional) · change · Approval pending* and never says "booked" before the quarter is published. The headline is the finding's plain name from `rules/rationale.yaml` ("Missing quarter-end share price"), followed by the engine's two or three `points` with stub references and rule codes stripped; below it, two columns — *Why flagged* (the points, an Evidence button) and *Suggested next step* (the recommendation as one sentence, an *AI draft* or *Policy suggestion* label, the number, the reasoning and the other priced options behind a disclosure). The bottom bar carries one primary action that addresses the actual blocker — **Add closing price** on a listed position with no measurement-date quote, otherwise the suggested step under a short verb — and **Override** as the secondary route, whose `$M` input appears only when clicked. *Evidence & history* folds away the rule codes with their names, the stand-in sentence, the price source on file, the decision on record, approval and publication state, last quarter's flags and the flag history.

Every route ends in the same confirmation, which will not close without a name and a reason. *Add closing price* takes the quarter-end market cap (or price × shares) and its source, derives the mark from HC's stake, and records the decision as `<rule>/price` with the evidence attached to the ledger record (`OverrideRecord.evidence`); the API refuses closing-price evidence without a positive market cap and a named source. *Accept proposed mark* records `<rule>/proposed`; a typed number records `<rule>/manual` with the reason starting empty. On a finding that says an input is missing, a decision that does not supply it cannot be confirmed until the approver ticks an acknowledgement, and the reason is prefixed `[input still missing]` — an override may step past required evidence, but never silently. The E-01 audit step records the route (`chosen_by`: *engine option*, *proposed mark accepted*, *mark entered by the approver*, *closing price supplied*, or *custom override* for a hand-written record) and, for a price, the evidence in words, so the audit trail export, the detail panel's chain and the executive dashboard's committee-overrides section say not just what was recorded but how the number was arrived at.

**One recommendation per flag.** The reviewer sees one resolution first — `Flag.recommendation` — chosen among those priced suggestions by `recommend.py`, which runs in the pipeline after the engine (the engine never holds a client). Two choosers, set by `recommendation.provider` in the policy or `--recommender`: `policy` takes the rule's own default (its first suggestion, worded as the rule wrote it); `claude` sends the case brief — the position, the flag, the audit steps, any vendor signals, and the candidates with their booked numbers — to the Anthropic SDK and asks it to choose one candidate by key and explain the choice for this company. The reply is validated hard (the choice must be a candidate key; the wording must fit the card), the booked number is taken from the chosen candidate and never from the reply, and every answer is cached under `data/recommendations/<sha>.json` keyed by the brief, the prompt and the model, so a rerun is deterministic and offline. Without an API key or a cached answer it falls back to `policy` and the recommendation says so. `hc-valuation recommend` fills the cache; the manifest records `recommender` (`policy` or `claude:<model>`). The other candidates stay one click away ("Other engine-priced options"), and a recommendation is accepted exactly as any suggestion is — under a named approver, as an E-01 override.

Flag ids as implemented: X-101 non-mechanical treatment (BLOCK for IPO, announced acquisition, closed exit with no proceeds; REVIEW when exit proceeds differ from ownership × deal value), X-102 down round (BLOCK), X-103 non-participation dilution (MONITOR), X-104 secondary price spread (REVIEW), X-105 note-language screen (REVIEW), X-106 flat extension without price discovery (REVIEW), X-107 funded bridge note (REVIEW), X-108 unfunded bridge note (MONITOR), X-109 term sheet disclosure (MONITOR), X-124 note converted in the round (REVIEW), X-125 term sheet withdrawn (REVIEW), X-126 operating metrics reported in prose (REVIEW), X-201/202 staleness (MONITOR > 24 mo / REVIEW > 48 mo), X-301/302 ARR contraction (MONITOR < 0 / REVIEW < −15%), X-303/304 runway aged one month (MONITOR < 12 / REVIEW < 6), X-401/402 implied multiple high/low (MONITOR, both directions; absolute 30× / 3× bounds under this policy), X-403 ARR below the screening floor (MONITOR), X-404 MOIC > 5× on a stale round (MONITOR), X-405 stale price that a live screen argues with (REVIEW, above), E-01 override drift (REVIEW), E-07 open item past its ageing limit (REVIEW), M-999 unrecognised event (BLOCK). X-9xx are ingestion issues raised before the engine runs and carried in `ValuationRun.validation`.

---

## 8. Engine components E-01 … E-08
...
**A rule for an event no rule covers** (`engine/declarative.py`, `engine/dsl.py`, `custom_rules:` in the policy file). When M-999 halts on an event type with no handler the position blocks at its prior mark and nothing is booked. A rule for the new type is written into the policy file by a person as a declarative formula, parsed against the `declarative:` whitelist, carrying an approver and an `effective_from` date; the next run applies it and the audit chain names it. Drafting that rule automatically — the analogue rule, the formula, the facts still missing, as a proposal a person ratifies — was built and has been removed from this deliverable; it is section 11, item 5.
...
- **Relative multiple thresholds.** `multiple.mode: relative_to_comps`, gated to sectors whose comps are an observed public history: the X-401/402 bounds are 2.0× / 0.5× the live sector median; a sector on the fixture keeps the absolute 30× / 3× (and the $0.5M ARR floor applies everywhere).
- **The stub comp history trends hard.** With the live-history gate waived on the PitchBook-shaped fixture, 31 of the 42 M-080 candidates pin at the +35% bound and two more at −35%, so on the fixture it is the bound, not the comps, that sets most calibrated alternatives. That is a property of the fixture, not the rule, and the reason `calibration.require_live_history` is on: the fixture never calibrates; the live feed does. See the M-080 section of the policy document.

Dispositions on the illustrative fixture (`--provider stub`): **7 BLOCK / 20 REVIEW / 39 MONITOR / 34 CLEAR** (blocked: Drayvenn, Gryphonel, Oakenvale, Tarnwick Aerospace, Duskfern, Birchhollow, Pellagrin). On the committed live comps, the shipped default, the X-401/402 screens re-bound and X-405 escalates Beltrix and Jupelan: **9 / 25 / 44 / 22**. The policy document's original hand count of 6 blocks predates X-106, which puts Pellagrin's flat extension and its 59-month staleness in two REVIEW families; the count of 15 REVIEW predates X-405 (section 7).
...
| M-080 comps calibration | **Built and on**, gated to an observed comps history (`calibration.require_live_history: true`): under `--provider live` every stale Level 3 position whose sector history covers its round month gets a `calibrated_to_comps` alternative; the fixture never calibrates. The Market page lists every calibrated position (round month, comps then → now, factor, bound hit), and the `comps_move` block beside the ±20% shock applies each sector's observed quarter move to the marks it drives. | On the fixture most candidates would pin at the bound (section 9), so a live history is the only honest input; the EDGAR extraction reads each filer's own reported periods across every revenue concept it has used, so January/April fiscal years and concept switches (NVIDIA, C3.ai, SoundHound) price correctly, and Yahoo history runs 10 years so the round months are covered. With the committed cache (55 months, 2022-03 to 2026-09, all 12 sectors live) M-080 produces 42 indications, 24 of them set by the ±35% limit on raw ratios of 0.32×–2.70× — the limit binding over five-name baskets, with the raw ratio shown beside the bounded one on every step. |
...
5. **A drafted treatment for an event no rule covers.** M-999 blocks the position at its prior mark and a person writes the rule into the policy file (`custom_rules:`, a declarative formula parsed against the `declarative:` whitelist, approved and dated). The next step is to draft that rule automatically from the event and the rule catalogue — the analogue rule it reasons from, the formula, and the facts still missing — as a proposal a person accepts before it prices anything. The mechanism it would write into already exists and is exercised by tests; only the drafting is absent, and nothing books a number without a named approval either way.
6. Beyond those: escrow and holdback as receivables on closed exits (the schema supports it; no Q3 event needed it), and a continuation-vehicle or structured-secondary rule once a real case defines it — added as a policy version bump, not a quiet mid-quarter change.
...
- **Idempotency is already real** and needs no work: the run id is `sha256(input | policy content | engine | decisions and carried items | market source)`, the same inputs produce a byte-identical run (`tests/test_determinism.py`, the golden test), and a re-fired trigger reproduces rather than duplicates.
...
A model sits *inside* the product in two places, both with the same guard. The note reader: with `note_reader.provider: claude`, each activity row's free text is read against the case catalogue and classified — quoted, never priced — and the engine turns the reading into review findings (X-126, X-130, X-131, X-132) that only ever add to what a reviewer sees. The recommender: with `recommendation.provider: claude`, the model chooses which of the engine's already-priced resolutions to put first on a BLOCK/REVIEW flag and writes the sentence and two reasons for that company's facts (§8, "One recommendation per flag"); the choice must name a candidate, the number comes from the engine, the answer is cached for determinism, and a named person still confirms. No model output is a booked mark anywhere in the system.

**Reuse and open-source.** No prior code of Jackson's was reused; everything in this repository was written for this exercise. Open-source components, by area — Python: FastAPI and uvicorn (the API and server), Typer (the CLI), pydantic (the value objects and the strict policy loader), openpyxl (the workbook in and out), PyYAML (the policy and the ledgers), httpx (the live feed, `live` extra) and the optional `anthropic` SDK (the note reader and the recommender, `claude` extra). Front ends: React, Vite, TypeScript, Tailwind CSS, TanStack Table (the review tool's tables) and Recharts (both dashboards' charts). Tests: pytest; Playwright drives the screenshot scripts in `frontend/screenshot.py` and `frontend-exec/screenshot.py` that produced the light/dark captures, and is not a test dependency.
