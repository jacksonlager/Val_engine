...
**Readiness: Ready.** Approval pending. It appears on New Activity with the row and the arithmetic, and on the Queue in the Ready bucket. Nobody has to do anything except read it.
...
Work New Activity, then the Queue. When nothing is Blocked or Needs Review, **Publish** from the header (name required; a second name if the policy asks). Then:
